...
# ╔═╡ eb5d3c2b-6ff3-435b-9a8a-3e450a5bfe15
begin
	begin
		import Pkg
		using CSV, DataFrames,Printf, CairoMakie, ColorSchemes, PlutoUI, RDatasets, Statistics, StatsBase
	end
end
...
# ╔═╡ e81d739c-6373-4eca-8f82-60e509d22268
md"packages"
...
url = "https://raw.githubusercontent.com/data-8/textbook/main/assets/data/bta.csv"

# ╔═╡ 3a1ee822-c0a2-4510-b703-d0b5b194c3db
download(url, "data.csv")

# ╔═╡ f4f72235-7bcd-4eba-ac50-eaf6ed9b2c44
begin
	
	back_pain_data = CSV.read("data.csv", DataFrame)
end

# ╔═╡ 885ae1cb-38d3-4480-af83-e8bc47f06cd6
begin
	using Random
	#rng  = back_pain_data[:,:Group]
	Sim_Group_under_Null = shuffle(back_pain_data[:,:Group])
end
...
# ╔═╡ 7d5e45e6-a9ff-4f55-87e5-3adc018b35e3
function relief(col)
	sum(col)/length(col)
end

# ╔═╡ 2b4e1258-0e3a-11eb-3d68-554a7486c259
trial = combine(groupby(back_pain_data, "Group"), :Result => (col -> relief(col)) => "Fraction with pain relief")
...
# ╔═╡ db71fb35-81e6-42c1-aeb9-064e2258eb90
function group(trial)
    fig = Figure()

    xs = 1:2
    ys = [0.125, 0.6]
    grp = ["Control","Treatment"]
    axis = Axis(fig[1,1],xticks = (1:2,grp), ylabel = "Fraction with pain relief", title = "Back Treatment Data")
	barplot!(axis, xs, ys, 
        color=xs,
        bar_labels= :y, 
        width = 0.9 )
...
# ╔═╡ 3efbd677-40bf-4cfd-b091-f37f462d8864
group(trial)
...
md"
_null hypothesis_: The drug test didn't work and there wasn't really any difference between the treatment and control. the outcome was the same regardless of the group each patients were assigned to

_alternative hypothesis_: The drug test does something different from the control!!!
"
...
md"
_test statistic_:=  the distance between the two group proportions, that is, the absolute value of the difference between them. |Gₜ - Gₙ| where Gₜ is the group proportion for the treatment and Gₙ the group proportion for control.
...
# ╔═╡ 5339ab36-a06f-433c-8447-6e5c19106f08
rng  = back_pain_data[:,:Group]

# ╔═╡ ef110891-15a7-4249-bbe1-679fcb92b34c
shuffle(back_pain_data[:,:Group])

# ╔═╡ 09ef7441-791f-4e86-84ed-c123a35e053d
back_pain_data[:,:Group]

# ╔═╡ 3eae9a47-ee77-4ecf-94c1-9afc4498a610
begin
	back_pain_data[:,"Sim_Group_under_Null"] = Sim_Group_under_Null
	back_pain_data
end
...
function proportion_w_pain_relief(data::DataFrame, group_col::String, group::String)
	observed_proportions = combine(groupby(data, group_col), :Result => (col -> relief(col)) => "Fraction with pain relief")
	if group == "Control"
		return observed_proportions[:,"Fraction with pain relief"][1]
	else
		return observed_proportions[:,"Fraction with pain relief"][2] end
end
...
proportion_w_pain_relief(back_pain_data,"Group","Control")
...
# ╔═╡ 18b41ba7-a009-4657-93d5-e45353b34c67
function test_statistics(data::DataFrame, group_col::String)
	T = proportion_w_pain_relief(data,group_col,"Treatment")
	C = proportion_w_pain_relief(data,group_col,"Control")
	observed_distance = abs(T - C)
...
begin
	Gₐ = test_statistics(back_pain_data,"Group")
	#Gₒ = test_statistics(back_pain_data,"Sim_Group_under_Null")
end
...
# ╔═╡ a781751e-db61-49e4-b68b-263df8e611d4
begin
	repetitions = 10000
	countss = zeros(repetitions)
	for i in 1:length(collect(1:repetitions))
		rng  = back_pain_data[:,:Group]
		Sim_Group_under_Null = shuffle(rng)
		back_pain_data[:,"Sim_Group_under_Null"] = Sim_Group_under_Null
		countss[i] = test_statistics(back_pain_data,"Sim_Group_under_Null")
		
	end
end

# ╔═╡ b0919e3f-6fcd-4ada-b29e-5ca9522d1812
sum(countss.>=0.475)/length(countss)

# ╔═╡ 4c5d30c2-0e59-11eb-3987-3bc356bb1f66
begin
	local fig = Figure()
	local ax = Axis(fig[1, 1], 
		title="Distribution of Test Statistics", 
		xlabel="Test_statistics",
	    ylabel="Height"	)
	hist!(countss, color="blue",bins = 5,binwidth = 0.1)
	vlines!([0.475], width=0.3, color=:red, linestyle = :dashdot)
	 fig
end

# ╔═╡ aef8f34c-badb-42ac-b042-1feb70b33644
begin
	p_sum = sum(countss.>=0.475)
	
	p_value = p_sum/length(countss) * 100
end

# ╔═╡ 6d21aa0c-d35b-4668-b53a-e956acab8530
...
md"the p-value is small (0.86%). The p value which is meant to signify the probability of obtaining the observed values, assuming the null hypothesis we made earlier is true: which is the treatment didnt have any effect at all. however, a low p-value signals that, if the null hypothesis is true, the probability of obtaining our actual observed values would be very low."

# ╔═╡ 66ca616c-0e59-11eb-30e1-132f15afa55d
md"
🐶 do you reject or accept the null hypothesis?

from the value i arrived at, i reject the null hypothesis

🐶 what level of significance are you using, and why?

i decided to use a significance level of 1% because the smaller the p-value, the stronger the evidence for rejecting the null hypothesis (treatment had no effect) and also confirming our outcome  as being statistically significant. In the medical field, it is expected that every trial drug must have any effect while being tested and observed.
...
i chose a two sided hypothesis because i needed to compare my results. (basically, if not this, then this and vice versa) and also because -all statistical test- attempt to choose between two views of the world. 
the differences were basic in this context, that one view dictates that no effect was felt at all from a drug test while the second dictates that any effect (good or bad) was felt.
...
yes, this is evidence for causality because the trail was randomized. true, it could still be due to chance, but unlikely.
...
no, it wouldnt. if the patients here were allowed to choose the control and treatment group then the outcome would have created a confounding variable, which is an uncalculated variable that influences the result of a cause and effect relationship. an example of a confounding variable created would be that a patient with pain would choose the treatment group totally ignoring the possibility that the patient in question could experience a relief even without the drug, hence distorting the results and including bias in the system.
"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "e0e2de9185e134a6760a3e58bdd0f0cc9f5db6b6"
